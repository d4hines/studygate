...
- [routing.cljs](https://github.com/d4hines/studygate/blob/dynamics/src/main/studygate/routing.cljs) hooks up HTML5 routing to the routing mutations in `api.cljs`. 
    - In [line 38](https://github.com/d4hines/studygate/blob/dynamics/src/main/studygate/routing.cljs#L36) `configure-routing!` closes of the app's root-level "reconciler".
        - This is the very easiest way of doing routing, but admittedly NOT the best way, as every routing action triggers an app-level re-render. Fulcro provides it's own idiomatic way of routing (union queries) to alleviate this.
...
Many thanks to Eric Normand and Tony Kay for their time, and to guys on Clojurian #beginners and #fulcro for enduring my pestering.
